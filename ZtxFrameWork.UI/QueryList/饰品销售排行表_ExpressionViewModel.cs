...
{
    [POCOViewModel]
    public class 饰品销售排行表_ExpressionViewModel : EntityExpressionBuilder<销售单明细>
    {
        public static 饰品销售排行表_ExpressionViewModel Create(IEnumerable<string> hiddenProperties, IEnumerable<string> additionalProperties)
        {
            return ViewModelSource.Create(() => new 饰品销售排行表_ExpressionViewModel(hiddenProperties, additionalProperties));
        }

        protected 饰品销售排行表_ExpressionViewModel(IEnumerable<string> hiddenProperties, IEnumerable<string> additionalProperties) : base(hiddenProperties, additionalProperties)
        {
            Init();
        }
