...
{
    [POCOViewModel]
    public class 盈亏单明细表_ExpressionViewModel : EntityExpressionBuilder<盈亏单明细>
    {
        public static 盈亏单明细表_ExpressionViewModel Create(IEnumerable<string> hiddenProperties, IEnumerable<string> additionalProperties)
        {
            return ViewModelSource.Create(() => new 盈亏单明细表_ExpressionViewModel(hiddenProperties, additionalProperties));
        }

        protected 盈亏单明细表_ExpressionViewModel(IEnumerable<string> hiddenProperties, IEnumerable<string> additionalProperties) : base(hiddenProperties, additionalProperties)
        {
            Init();
        }
        ...
        {
            this.AdvancedExpression = CriteriaOperatorToExpressionConverter.GetEntityFrameworkWhere<盈亏单明细>(this.AdvancedCriteria);
            Expression<Func<盈亏单明细, bool>> d;
            if (操作员ID.HasValue)
            {
                d = (x) => x.盈亏单.操作员ID == 操作员ID.Value;
                this.AdvancedExpression = this.AdvancedExpression.And(d);
            }
            if (分店ID.HasValue)
            {
                d = (x) => x.盈亏单.分店ID == 分店ID.Value;
                this.AdvancedExpression = this.AdvancedExpression.And(d);
            }
          
            d = (x) => x.盈亏单.日期 >= 开始日期 && x.盈亏单.日期 <= 结束日期;
            this.AdvancedExpression = this.AdvancedExpression.And(d);
        }
