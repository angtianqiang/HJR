...
{
    [POCOViewModel]
    public class 付款明细表_ExpressionViewModel : EntityExpressionBuilder<付款单明细>
    {
        public static 付款明细表_ExpressionViewModel Create(IEnumerable<string> hiddenProperties, IEnumerable<string> additionalProperties)
        {
            return ViewModelSource.Create(() => new 付款明细表_ExpressionViewModel(hiddenProperties, additionalProperties));
        }

        protected 付款明细表_ExpressionViewModel(IEnumerable<string> hiddenProperties, IEnumerable<string> additionalProperties) : base(hiddenProperties, additionalProperties)
        {
            Init();
        }
        ...
        protected override void SetLoadData()
        {
            ...
            操作员ID = 分店ID = 供应商ID = null;
        }
        protected override void BuilderExpression()
        {
            this.AdvancedExpression = CriteriaOperatorToExpressionConverter.GetEntityFrameworkWhere<付款单明细>(this.AdvancedCriteria);
            Expression<Func<付款单明细, bool>> d;
            if (操作员ID.HasValue)
            {
              d= (x) => x.入库单.操作员ID== 操作员ID.Value;
                this.AdvancedExpression = this.AdvancedExpression.And(d);
            }
            if (分店ID.HasValue)
            {
              d = (x) => x.入库单.分店ID == 分店ID.Value;
                this.AdvancedExpression = this.AdvancedExpression.And(d);
            }
            if (供应商ID.HasValue)
            {
                d = (x) => x.入库单.供应商ID == 供应商ID.Value;
                this.AdvancedExpression = this.AdvancedExpression.And(d);
            }
           d = (x) => x.付款单.日期>= 开始日期 && x.付款单.日期<= 结束日期;
            this.AdvancedExpression = this.AdvancedExpression.And(d);
        }
