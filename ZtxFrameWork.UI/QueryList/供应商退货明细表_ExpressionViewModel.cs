...
{
    [POCOViewModel]
    public class 供应商退货明细表_ExpressionViewModel : EntityExpressionBuilder<退库单明细>
    {
        public static 供应商退货明细表_ExpressionViewModel Create(IEnumerable<string> hiddenProperties, IEnumerable<string> additionalProperties)
        {
            return ViewModelSource.Create(() => new 供应商退货明细表_ExpressionViewModel(hiddenProperties, additionalProperties));
        }

        protected 供应商退货明细表_ExpressionViewModel(IEnumerable<string> hiddenProperties, IEnumerable<string> additionalProperties) : base(hiddenProperties, additionalProperties)
        {
            Init();
        }
        ...
        {
            this.AdvancedExpression = CriteriaOperatorToExpressionConverter.GetEntityFrameworkWhere<退库单明细>(this.AdvancedCriteria);
            Expression<Func<退库单明细, bool>> d;
            if (操作员ID.HasValue)
            {
                d = (x) => x.退库单.操作员ID == 操作员ID.Value;
                this.AdvancedExpression = this.AdvancedExpression.And(d);
            }
            if (分店ID.HasValue)
            {
                d = (x) => x.退库单.分店ID == 分店ID.Value;
                this.AdvancedExpression = this.AdvancedExpression.And(d);
            }
            //if (会员ID.HasValue)
            //{
            ...
            if (供应商ID.HasValue)
            {
                d = (x) => x.退库单.供应商ID == 供应商ID.Value;
                this.AdvancedExpression = this.AdvancedExpression.And(d);
            }

            d = (x) => x.退库单.日期 >= 开始日期 && x.退库单.日期 <= 结束日期;
            this.AdvancedExpression = this.AdvancedExpression.And(d);
        }
