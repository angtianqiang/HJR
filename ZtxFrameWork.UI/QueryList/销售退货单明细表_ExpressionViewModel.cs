...
{
    [POCOViewModel]
    public class 销售退货单明细表_ExpressionViewModel : EntityExpressionBuilder<销售退货单明细>
    {
        public static 销售退货单明细表_ExpressionViewModel Create(IEnumerable<string> hiddenProperties, IEnumerable<string> additionalProperties)
        {
            return ViewModelSource.Create(() => new 销售退货单明细表_ExpressionViewModel(hiddenProperties, additionalProperties));
        }

        protected 销售退货单明细表_ExpressionViewModel(IEnumerable<string> hiddenProperties, IEnumerable<string> additionalProperties) : base(hiddenProperties, additionalProperties)
        {
            Init();
        }
        ...
        {
            this.AdvancedExpression = CriteriaOperatorToExpressionConverter.GetEntityFrameworkWhere<销售退货单明细>(this.AdvancedCriteria);
            Expression<Func<销售退货单明细, bool>> d;
            if (操作员ID.HasValue)
            {
                d = (x) => x.销售退货单.操作员ID == 操作员ID.Value;
                this.AdvancedExpression = this.AdvancedExpression.And(d);
            }
            if (分店ID.HasValue)
            {
                d = (x) => x.销售退货单.分店ID == 分店ID.Value;
                this.AdvancedExpression = this.AdvancedExpression.And(d);
            }
            if (会员ID.HasValue)
            {
            ...
            //if (供应商ID.HasValue)
            //{
            //    d = (x) => x.销售退货单.供应商ID == 供应商ID.Value;
            //    this.AdvancedExpression = this.AdvancedExpression.And(d);
            //}
            d = (x) => x.销售退货单.日期 >= 开始日期 && x.销售退货单.日期 <= 结束日期;
            this.AdvancedExpression = this.AdvancedExpression.And(d);
        }
