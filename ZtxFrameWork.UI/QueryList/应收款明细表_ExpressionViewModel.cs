...
{
    [POCOViewModel]
    public class 应收款明细表_ExpressionViewModel : EntityExpressionBuilder<销售单>
    {
        public static 应收款明细表_ExpressionViewModel Create(IEnumerable<string> hiddenProperties, IEnumerable<string> additionalProperties)
        {
            return ViewModelSource.Create(() => new 应收款明细表_ExpressionViewModel(hiddenProperties, additionalProperties));
        }

        protected 应收款明细表_ExpressionViewModel(IEnumerable<string> hiddenProperties, IEnumerable<string> additionalProperties) : base(hiddenProperties, additionalProperties)
        {
            Init();
        }
        ...
        protected override void SetLoadData()
        {
            ...
            操作员ID = 分店ID = 会员ID = null;
        }
        protected override void BuilderExpression()
        {
            this.AdvancedExpression = CriteriaOperatorToExpressionConverter.GetEntityFrameworkWhere<销售单>(this.AdvancedCriteria);
            Expression<Func<销售单, bool>> d;
            if (操作员ID.HasValue)
            {
                d = (x) => x.操作员ID == 操作员ID.Value;
                this.AdvancedExpression = this.AdvancedExpression.And(d);
            }
            if (分店ID.HasValue)
            {
                d = (x) => x.分店ID == 分店ID.Value;
                this.AdvancedExpression = this.AdvancedExpression.And(d);
            }
            if (会员ID.HasValue)
            {
                d = (x) => x.会员ID == 会员ID.Value;
                this.AdvancedExpression = this.AdvancedExpression.And(d);
            }
            d = (x) => x.日期 >= 开始日期 && x.日期 <= 结束日期;
            this.AdvancedExpression = this.AdvancedExpression.And(d);
        }
