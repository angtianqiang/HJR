...
namespace ZtxFrameWork.UI.ViewModels
{
    ...
    public class 付款单CollectionViewModel : CollectionViewModel<付款单, ZtxDB, long>
    {
        ...
        protected 付款单CollectionViewModel() : base(DbFactory.Instance, x => x.付款单s, query => query.Include(t => t.分店).Include(t => t.操作员).Include(t=>t.供应商).OrderBy(x=>x.编号), x =>x.ID,t=>InitEntity(t), permissionTitle: "付款单", allowPage: true)
        {
        ...
        {
         var t1=   GetNewCode(App.Current分店.编号 + "FK", DbFactory.Instance, x => x.付款单s, t => t.编号);
        ...
        protected override void OnBeforeEntityDeleted(ZtxDB dbContext, long primaryKey, 付款单 entity)
        {
            ...
            if (!dbContext.Entry(entity).Collection(t => t.付款单明细s).IsLoaded)
            {
                dbContext.Entry(entity).Collection(t => t.付款单明细s).Load();
            }
