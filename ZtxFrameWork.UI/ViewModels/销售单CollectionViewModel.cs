...
namespace ZtxFrameWork.UI.ViewModels
{
    ...
    public class 销售单CollectionViewModel : CollectionViewModel<销售单, ZtxDB, long>
    {
        ...
        protected 销售单CollectionViewModel() : base(DbFactory.Instance, x => x.销售单s, query => query.Include(t => t.会员).Include(t => t.分店).Include(t => t.操作员).OrderByDescending(x => x.编号).Take(App.ViewTopCount), x =>x.ID,t=>InitEntity(t), permissionTitle: "销售单",allowPage:true)
        {
        ...
        {
            var t1 = GetNewCode(App.Current分店.编号 + "XS", DbFactory.Instance, x => x.销售单s, t => t.编号);
