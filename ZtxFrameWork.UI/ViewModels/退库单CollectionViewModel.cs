...
namespace ZtxFrameWork.UI.ViewModels
{
    ...
    public class 退库单CollectionViewModel : CollectionViewModel<退库单, ZtxDB, long>
    {
        ...
        protected 退库单CollectionViewModel() : base(DbFactory.Instance, x => x.退库单s, query => query.Include(t => t.供应商).Include(t => t.分店).Include(t => t.操作员).OrderByDescending(x => x.编号).Take(App.ViewTopCount), x =>x.ID,t=>InitEntity(t), permissionTitle: "采购退货单", allowPage: true)
        {
        ...
        {
            var t1 = GetNewCode(App.Current分店.编号 + "TK", DbFactory.Instance, x => x.退库单s, t => t.编号);
        ...
        protected override void OnBeforeEntityDeleted(ZtxDB dbContext, long primaryKey, 退库单 entity)
        {
            ...
            if (!dbContext.Entry(entity).Collection(t => t.退库单明细s).IsLoaded)
            {
                dbContext.Entry(entity).Collection(t => t.退库单明细s).Load();
            }
