...
namespace ZtxFrameWork.UI.ViewModels
{
    ...
    public class 销售退货单CollectionViewModel : CollectionViewModel<销售退货单, ZtxDB, long>
    {
        ...
        protected 销售退货单CollectionViewModel() : base(DbFactory.Instance, x => x.销售退货单s, query => query.Include(t=>t.会员).Include(t=>t.分店).Include(t=>t.操作员).OrderByDescending(x => x.编号).Take(App.ViewTopCount), x =>x.ID,t=>InitEntity(t), permissionTitle: "销售退货单", allowPage: true)
        {
        ...
        {
            var t1 = GetNewCode(App.Current分店.编号 + "TH", DbFactory.Instance, x => x.销售退货单s, t => t.编号);
        ...
        protected override void OnBeforeEntityDeleted(ZtxDB dbContext, long primaryKey, 销售退货单 entity)
        {
            ...
            if (!dbContext.Entry(entity).Collection(t => t.销售退货单明细s).IsLoaded)
            {
                dbContext.Entry(entity).Collection(t => t.销售退货单明细s).Load();
            }
