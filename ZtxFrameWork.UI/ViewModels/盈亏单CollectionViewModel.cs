...
namespace ZtxFrameWork.UI.ViewModels
{
    ...
    public class 盈亏单CollectionViewModel : CollectionViewModel< 盈亏单, ZtxDB, long>
    {
        ...
        protected  盈亏单CollectionViewModel() : base(DbFactory.Instance, x => x. 盈亏单s, query => query.Include(t=>t.分店).Include(t=>t.操作员).OrderByDescending(x => x.编号).Take(App.ViewTopCount), x =>x.ID,t=>InitEntity(t), permissionTitle: "盈亏单", allowPage: true)
        {
        ...
        {
            var t1 = GetNewCode(App.Current分店.编号+"YK", DbFactory.Instance, x => x.盈亏单s, t => t.编号);
