...
namespace ZtxFrameWork.UI.ViewModels
{
    ...
    public class 入库单CollectionViewModel : CollectionViewModel<入库单, ZtxDB, long>
    {
        ...
        protected 入库单CollectionViewModel() : base(DbFactory.Instance, x => x.入库单s, query => query.Include(t => t.供应商).Include(t => t.分店).Include(t => t.操作员).OrderByDescending(x => x.编号).Take(App.ViewTopCount), x => x.ID, t => InitEntity(t), permissionTitle: "采购入库单")
        {
