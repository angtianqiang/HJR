...
namespace ZtxFrameWork.UI.ViewModels
{
    ...
    public class 分店库存ViewModel : CollectionViewModel<库存, ZtxDB, long>
    {
        ...
        protected 分店库存ViewModel() : base(DbFactory.Instance, x => x.库存s, query => query.Include(t=>t.饰品).Include(t => t.饰品.品名).Include(t => t.饰品.饰品类型).Include(t => t.饰品.材质).Include(t => t.饰品.电镀方式).Include(t => t.饰品.石头颜色).Include(t=>t.分店).OrderByDescending(x =>x.饰品.编号), x =>x.ID, permissionTitle: "分店库存")
        {
            Init();
        }
        public void Init()
